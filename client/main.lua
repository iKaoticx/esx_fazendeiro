...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		local elements = {
			{label = 'Trator',  value = 'Tractor2'},
			{label = 'Caminhão de transporte',  value = 'Benson'},
		}
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_farm', 'Farmer', amount)
								end)
							end
						end
					end,
...
function blips()
	if publicBlip == false then
		...
		AddTextComponentString("Fazenda Jhonson's")
